...
class SkillGUI:
    ...
    #             )
    #             if reply:
    #                 self._connected = reply.data["connected"]
    ...
    def build_message_type(self, event, page):
        """Builds a message matching the output from the enclosure."""
        return f"{self.skill.skill_id}.{page}.{event}"
    ...
    def register_handler(self, event, page, handler):
        ...
        """
        msg_type = self.build_message_type(event, page)
