...
class Api:
    ...
    params_to_etag: typing.Dict[str, typing.Any] = {}
    etag_to_response: typing.Dict[str, typing.Any] = {}
...
class DeviceApi(Api):
    ...
    def activate(self, state, token):
        ...
        return self.request(dict(method="POST", path="/activate", json=request_data))

    def sync_pantacor_config(self, pantacor_device_id):
        request_data = dict(
            mycroft_device_id=self.identity.uuid, pantacor_device_id=pantacor_device_id
        )
        return self.request(dict(method="POST", path="/pantacor", json=request_data))
