Feature: Timer Status
  ...
  Scenario Outline: status of a single timer
    ...
    Then "mycroft-timer.mycroftai" should reply with dialog from "time-remaining"
  ...
  Scenario Outline: status of named timer
    ...
    When the user says "<timer status request>"
    Then "mycroft-timer.mycroftai" should reply with dialog from "time-remaining-named"
  ...
  Scenario Outline: status of two timers
    ...
    And multiple active timers
      ...
      | 10 minutes |
    When the user says "<timer status request>"
    Then "mycroft-timer.mycroftai" should reply with dialog from "number-of-timers"
    And "mycroft-timer.mycroftai" should reply with dialog from "time-remaining-named"
    And "mycroft-timer.mycroftai" should reply with dialog from "time-remaining-named"
