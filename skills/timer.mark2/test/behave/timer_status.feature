Feature: Timer Status
  ...
  Scenario Outline: status of a single timer
    ...
    Then "timer.mark2" should reply with dialog from "time-remaining"
  ...
  Scenario Outline: status of named timer
    ...
    When the user says "<timer status request>"
    Then "timer.mark2" should reply with dialog from "time-remaining-named"
    And timers are stopped
  ...
  Scenario Outline: status of two timers
    ...
    And multiple active timers
      ...
      | 10 minutes |
    When the user says "<timer status request>"
    Then "timer.mark2" should reply with dialog from "number-of-timers"
    And "timer.mark2" should reply with dialog from "time-remaining-named"
    And "timer.mark2" should reply with dialog from "time-remaining-named"
    And timers are stopped
