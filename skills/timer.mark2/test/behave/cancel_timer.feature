Feature: Cancel Timers
  ...
  Scenario Outline: cancel a timer when only one timer is active
    ...
    Then "timer.mark2" should reply with dialog from "cancelled-single-timer"
  ...
  Scenario Outline: cancel a timer when multiple timers are active using duration
    ...
    And multiple active timers
      ...
      | 3 minutes |
    When the user says "<cancel timer request>"
    Then "timer.mark2" should reply with dialog from "ask-which-timer-cancel"
    And the user replies "1 minute"
    And "timer.mark2" should reply with dialog from "cancelled-timer-named"
    And timers are stopped
  ...
  Scenario Outline: attempt to cancel timer when there are no timers active
    ...
    When the user says "<cancel timer request>"
    Then "timer.mark2" should reply with dialog from "no-active-timer"
    And timers are stopped
  ...
  Scenario Outline: cancel a timer specifying duration
    ...
    And multiple active timers
      ...
      | 10 minutes |
    When the user says "<cancel timer request>"
    Then "timer.mark2" should reply with dialog from "cancelled-timer-named"
    And timers are stopped
  ...
  Scenario Outline: cancel all active timers
    ...
    And multiple active timers
      ...
      | 15 minutes |
    When the user says "<cancel timer request>"
    Then "timer.mark2" should reply with dialog from "cancel-all"
    And timers are stopped
